...
$(document).ready(function(){$(window).scroll(function(){50<$(this).scrollTop()?$("#back-to-top").fadeIn():$("#back-to-top").fadeOut()});[].slice.call(document.querySelectorAll('[data-bs-toggle="tooltip"]')).map(function(a){return new bootstrap.Tooltip(a)});$("img.thumbnail").each(function(){var a=this.width,c=this.height;$(this).attr("height",c);$(this).attr("width",a)});$.ajaxSetup({cache:!1});$("#query").keyup(function(){$("#queryResult").html("");searchField=$("#query").val();expression=new RegExp(searchField,
...
b.url+'" data-bs-toggle="tooltip" title="'+b.title+'" data-bs-placement="bottom">'+b.title+'</a><span class="text-muted"> ...'+c+'... </span><a href="'+b.url+'" data-bs-toggle="tooltip" title="'+b.title+'" data-bs-placement="bottom">read more &rArr;</a></li>')})})});$("#ModalSearch").on("shown.bs.modal",function(){$("#query").focus()});$("#lp_collection").DataTable({columnDefs:[{orderable:!1,targets:0}],order:[[1,"asc"]],oLanguage:{sSearch:"Search Albums:",sLengthMenu:"Display _MENU_ Albums",sInfo:"Showing _START_ to _END_ of _TOTAL_ Albums"},
...
$("a[data-mail]").on("click",function(){window.location="mailto:"+$(this).data("mail")+"@paulwillard.nz?subject=Contact from website"});$('a[target="_blank"]').each(function(){$(this).attr("rel","noreferrer noopener")});$('a[target="blank"]').each(function(){$(this).attr("rel","noreferrer noopener");$(this).attr("target","_blank")});
