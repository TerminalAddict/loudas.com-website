...
b.toFixed(2)+" L<br>\n                 <strong>Water to Add:</strong> "+a.toFixed(2)+" L"))});$("#sugarWashForm").on("submit",function(a){a.preventDefault();a=parseFloat($("#sugarWashvolume").val());var b=parseFloat($("#sugarWashog").val());isNaN(a)||isNaN(b)||0>=a||1>=b?$("#sugarWashresult").removeClass("alert-info").addClass("alert-danger").removeClass("d-none").text("Please enter valid values (OG must be > 1.000)."):(b=1E3*a*(b-1)/384,a-=b,$("#sugarWashresult").removeClass("d-none alert-danger").addClass("alert-info").html("<strong>Sugar Required:</strong> "+
b.toFixed(2)+" kg<br>\n                 <strong>Water Required:</strong> "+a.toFixed(2)+" L"))})});$("a[data-mail]").on("click",function(){window.location="mailto:"+$(this).data("mail")+"@paulwillard.nz?subject=Contact from website"});$('a[target="_blank"]').each(function(){$(this).attr("rel","noreferrer noopener")});$('a[target="blank"]').each(function(){$(this).attr("rel","noreferrer noopener");$(this).attr("target","_blank")});
