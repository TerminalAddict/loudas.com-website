...
$(document).ready(function(){$(window).scroll(function(){50<$(this).scrollTop()?$("#back-to-top").fadeIn():$("#back-to-top").fadeOut()});$("#back-to-top").click(function(){$("#back-to-top").tooltip("hide");$("body,html").animate({scrollTop:0},800);return!1});$("#back-to-top").mouseover(function(){$("#back-to-top").tooltip("show")});$("img.thumbnail").each(function(){var a=this.width,c=this.height;$(this).attr("height",c);$(this).attr("width",a)});$.ajaxSetup({cache:!1});$("#query").keyup(function(){$("#queryResult").html("");
searchField=$("#query").val();expression=new RegExp(searchField,"i");$.getJSON("/assets/search.json",function(a){$.each(a,function(c,b){if(-1!=b.url.search(expression)||-1!=b.content.search(expression)||-1!=b.title.search(expression))c=b.content.search(expression)-50,c=b.content.substring(c,c+300),""!=searchField&&(c=c.split(expression).join("<span class='shadow bg-primary text-white rounded px-1'>"+searchField+"</span>"),b.title=b.title.split(expression).join("<span class='shadow bg-primary text-white rounded px-1'>"+
searchField+"</span>")),$("#queryResult").append('<li class="list-group-item link-class"><img src="/assets/images/search-blue.svg" alt="search" class="searchIcon" /><a class="pl-5" href="'+b.url+'" data-toggle="tooltip" data-original-title="'+b.title+'" data-placement="bottom">'+b.title+'</a><span class="text-muted"> ...'+c+'... </span><a href="'+b.url+'" data-toggle="tooltip" data-original-title="'+b.title+'" data-placement="bottom">read more &rArr;</a></li>')})})});$("#ModalSearch").on("shown.bs.modal",
function(){$("#query").focus()});$("#lp_collection").DataTable({columnDefs:[{orderable:!1,targets:0}],order:[[1,"asc"]],oLanguage:{sSearch:"Search Albums:",sLengthMenu:"Display _MENU_ Albums",sInfo:"Showing _START_ to _END_ of _TOTAL_ Albums"},drawCallback:function(a){$('[data-toggle="tooltip"]').tooltip();$("table#lp_collection tr").hover(function(){$(this).find("td.album_name").removeClass("album_bg_gs").addClass("album_bg_nogs")},function(){$(this).find("td.album_name").removeClass("album_bg_nogs").addClass("album_bg_gs")})}});
...
$('a[target="_blank"]').each(function(){$(this).attr("rel","noreferrer noopener")});$('a[target="blank"]').each(function(){$(this).attr("rel","noreferrer noopener");$(this).attr("target","_blank")});$('img[rel="lightbox"]').on("click",function(){var a=$(this).attr("src").replace("thumbnails/","");$("#ModalImageLabel").html("Larger image ...");$("#lightboxModalBody").html('<img src="'+a+'" alt="" class="w-100" />');$("#lightboxModal").modal("show")});
...
a("#comment-form fieldset").each(function(){a(this).addClass("hidden")});c("<strong>Thanks for your comment!</strong> It will show on the site once it has been approved.")},error:function(d){console.log(d);a("#comment-form-submit").html("Submit Comment");a(".post__comments-form .js-notice").removeClass("bg-success").addClass("bg-danger");c("<strong>Sorry, there was an error with your submission.</strong> Please make sure all required fields have been completed and try again.");a(b).removeClass("disabled")}});
return!1})})(jQuery);
